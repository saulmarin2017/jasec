...
class LiquidacioMateriales extends StatefulWidget {
  List<Map<String, dynamic>> produtosRecibidos;

  LiquidacioMateriales(this.produtosRecibidos, {super.key});

  @override
  State<LiquidacioMateriales> createState() => _LiquidacioMaterialesState();
}

//Las liquidaciones de materiales utilizan los mismos servicios de transferencias de materiales, las mismas tablas y clases de servicios
class _LiquidacioMaterialesState extends State<LiquidacioMateriales> {
  int idBodegaOrigen = 0;
  int idBodegaDestino = 0;

  bool cargandoBodega = false;

  String? idTipoMaterial = "";

  int? codLiquidacion = 0;
  ...
  List<String> mapTipoMaterial = ["CONSUMO", "DEVOLUCION"];
                              ...
                              children: [
                                Expanded(
                                  ...
                                  child: ListaSeleccionTexto(
                                    textoEtiqueta: lbltipodematerial,
                                    tamanofuente: tamanoFuente10px,
                                    opciones: mapTipoMaterial,
                                    valorSeleccionado: (idTipoMaterial),
                                    onChanged: (String? tm) async {
                                      if (tm!.isNotEmpty &&
                                          idBodegaOrigen > 0 &&
                                          idBodegaDestino > 0 &&
                                          codLiquidacion == 0) {
                                          ...
                                          if (idBodegaOrigen > 0 &&
                                            ...
                                            await MostrarWidgetEnDialogo(
                                                    ...
                                                    AgregarMaterialesTransferencia(
                                                        ...
                                                        idBodegaOrigen
                                                            .toString()),
                                                    '$lbltransferenciamateriales (# $codLiquidacion)')
                                                .then((r) async {
                    ...
                    ExpansionPanel(
                      ...
                      headerBuilder: (context, isExpanded) {
                        return ListTile(
                          title: Etiqueta(
                            texto:
                                '$lblmateriales / $lblliquidacion (# $codLiquidacion)',
                            tipo: FontWeight.bold,
                            color: colorBlanco,
                            tamanoFuente: tamanoFuenteDefecto,
                          ),
                ...
                Expanded(
                    ...
                    child: Boton(
                      ...
                      onPressed: () async {
                        ...
                        if (confirmacion!) {
                          await eliminarLiquidacion().then((r) {
  ...
  Future<void> cargarProductos() async {
    ...
    try {
      productos = await transferencia.traerDetalleTransferencia(
          codLiquidacion.toString(), idBodegaOrigen.toString());
  ...
  Future<void> guardarTransferencia() async {
    await crearLiquidacion().then((r) async {
      if (codLiquidacion! > 0 && widget.produtosRecibidos.isNotEmpty) {
        await crearDetalleLiquidacion().then((x) async {
          if (codLiquidacion! > 0) {
            await cargarProductos();
          }
        });
  ...
  Future<void> crearLiquidacion() async {
    CrudTransferencias crearLiquidacion = CrudTransferencias();
    ...
    try {
      Map<String, dynamic> mapTransferencia = {
        ...
        "tipo_material": idTipoMaterial,
      };

      vm = await crearLiquidacion.guardarTransferencia(mapTransferencia);
      if (vm.idInsertado != null) {
        if (vm.idInsertado! > 0) {
          codLiquidacion = vm.idInsertado ?? 0;
          BarraMensaje(context).Mensaje(
              '$lbltransferencia $codLiquidacion, $lblGuardoInformacion',
  ...
  Future<void> eliminarLiquidacion() async {
    ...
    try {
      Map<String, dynamic> mapTransferencia = {
        "cod_enc_movimiento": codLiquidacion
      };
  ...
  Future<void> actualizarLiquidacion() async {
    CrudTransferencias crudTransferencia = CrudTransferencias();
    VmRespuestaPost vm = VmRespuestaPost();
  ...
  Future<void> crearDetalleLiquidacion() async {
    CrudTransferencias cls = CrudTransferencias();
    VmRespuestaPost vm = VmRespuestaPost();
    ...
    try {
      ...
      for (var p in widget.produtosRecibidos) {
        Map<String, dynamic> mapRequisicion = {
          "cod_enc_materiales": codLiquidacion,
